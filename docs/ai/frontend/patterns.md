...
Исключение — когда меняется **аргумент** запроса и показывать данные предыдущего аргумента нельзя. `data`
держит последний успешный результат независимо от аргумента, а `isLoading` при его смене остаётся `false`,
поэтому страница отдала бы детям чужие данные. Тогда — `currentData` и `isFetching && !currentData`: при
рефетче с тем же аргументом `currentData` сохраняется, и flash по-прежнему не возникает. Пример — `BookPage`
(`docs/ai/frontend/pages/books.md`).
