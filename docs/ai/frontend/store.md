...
- Состояние: `{ available: boolean, reading: boolean, entries: Record<url, MediaCacheEntryState>, subscribers: string[] }`
...
- `reading` — прогон идёт: чтение Cache API занимает заметное время (в Safari — секунды на большом кеше), и UI по этому флагу показывает ожидание, а не «состояние недоступно». Без активного Service Worker прогон не начинается, флаг остаётся `false`, и UI сразу сообщает о недоступности
...
### Отчёт об отказах (`features/api`)

`apiErrorSaverMiddleware` слушает отклонённые запросы RTK Query и отправляет их описание на `POST /log`
(он же `/api/log` — у RTK Query базовый URL `/api`; свой отказ пропускает, иначе получился бы цикл). Аргументы запроса перед отправкой проходят через
`redactSecrets`: поля, чьи имена похожи на секреты — пароль, токен, подпись Telegram-логина, — заменяются
заглушкой на любой глубине. Без этого неудачный вход отправлял бы в серверный журнал пароль, а неудачный
вход через Telegram — подпись, которой хватает, чтобы войти самому.
