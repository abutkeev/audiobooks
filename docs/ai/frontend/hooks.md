...
| `useWakeLock` | Предотвращение блокировки экрана при воспроизведении; вызывается только из `PlayerHost` — второй экземпляр запросил бы свой лок и снимал его из-под первого. Поддержку показывает отдельный экспорт `wakeLockSupported` |
| `useWebSocket` | Управление WebSocket-соединением |
| `useTitle` | Установка заголовка страницы (dispatch в store) |
| `useKeyboardShortcuts` | Привязка клавиатурных сочетаний; принимает `enabled` и работает только на странице активной книги, см. [player.md](player.md) |
