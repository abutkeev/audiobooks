...
- Отказы **проверки данных** называют причину кодом в теле ответа (`code`), а не только текстом
  (ошибки конфигурации бота кода не несут): `tg_hash_invalid` —
  Telegram не подтвердил данные, `tg_data_expired` — подпись старше суток, `tg_account_unknown` —
  аккаунт ни к кому не привязан. Текст в ответе английский и предназначен журналу; пользователю
  фронтенд показывает перевод по коду (`utils/getTelegramAuthError`)
